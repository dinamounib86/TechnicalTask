...
fun Modifier.scrollbar(
    ...
    return drawWithContent {
        ...
        val isContentScrollable = state.layoutInfo.totalItemsCount > state.layoutInfo.visibleItemsInfo.size
        if (!isContentScrollable) return@drawWithContent
...
/**
 * Renders a scrollbar for standard scrollable components using [ScrollState].
 *
 * Note: This modifier should be called before the verticalScroll() or horizontalScroll() modifier
 * to ensure proper functionality.
 *
 * <ul>
 *   <li> A scrollbar is composed of two components: a track and a knob. The knob moves across
 *        the track
 *   <li> The scrollbar appears automatically when the user starts scrolling and disappears
 *        after the scrolling is finished
 * </ul>
 *
 * @param state The [ScrollState] that has been passed into the scrollable component
 * @param horizontal If `true`, this will be a horizontally-scrolling (left and right) scroll bar,
 *                   if `false`, it will be vertically-scrolling (up and down)
 * @param alignEnd If `true`, the scrollbar will appear at the "end" of the scrollable composable it
 *                 is decorating (at the right-hand side in left-to-right locales or left-hand side
 *                 in right-to-left locales, for the vertical scrollbars -or- the bottom for
 *                 horizontal scrollbars). If `false`, the scrollbar will appear at the "start" of
 *                 the scrollable composable it is decorating (at the left-hand side in
 *                 left-to-right locales or right-hand side in right-to-left locales, for the
 *                 vertical scrollbars -or- the top for horizontal scrollbars)
 * @param thickness How thick/wide the track and knob should be
 * @param padding Edge padding to "squeeze" the scrollbar start/end in so it's not flush with the
 *                contents of the scrollable composable it is decorating
...
fun Modifier.scrollbar(
    ...
    val isContentScrollable = state.maxValue > 0
    if (!isContentScrollable) return Modifier

    require(thickness > 0.dp) { "Thickness must be positive" }
    require(knobCornerRadius >= 0.dp) { "Knob corner radius must be non-negative" }
    require(trackCornerRadius >= 0.dp) { "Track corner radius must be non-negative" }
    require(hiddenAlpha <= visibleAlpha) { "Hidden alpha must not be greater than visible alpha" }
    require(fadeInAnimationDurationMs >= 0) { "Fade in duration must be non-negative" }
    require(fadeOutAnimationDurationMs >= 0) { "Fade out duration must be non-negative" }
    require(fadeOutAnimationDelayMs >= 0) { "Fade out delay must be non-negative" }
    ...
    return drawWithContent {
        ...
        if (state.isScrollInProgress || alpha > 0f) {
            val (viewportSize, contentSize) = if (horizontal) {
                size.width to state.maxValue.toFloat()
            } else {
                size.height to state.maxValue.toFloat()
            }

            val normalizedScrollOffset = state.value / contentSize.coerceAtLeast(1f)
            val knobSize = (viewportSize / contentSize.coerceAtLeast(1f)) * viewportSize
            val knobPosition = normalizedScrollOffset * (viewportSize - knobSize) + padding.toPx()

            drawScrollbar(
                horizontal = horizontal,
                alignEnd = alignEnd,
                thickness = thickness,
                knobCornerRadius = knobCornerRadius,
...
private fun DrawScope.drawScrollbar(
    ...
    drawRoundRect(
        ...
        size = if (horizontal) {
            Size(viewportSize - padding.toPx() * 2, thickness.toPx())
        } else {
            Size(thickness.toPx(), viewportSize - padding.toPx() * 2)
        },
        alpha = alpha,
    ...
    drawRoundRect(
        ...
        alpha = alpha,
        cornerRadius = CornerRadius(knobCornerRadius.toPx())
    )
}
